...
import { getPublicPlansWithBookmarkStatus } from "@/app/actions/plans";
import AnimatedFormBox from "@/components/animated-form-box";
import PublicPlansTable from "./components/public-plans-table";
...
export async function generateMetadata({
  ...
  const t = await getTranslations("PublicPlansPage");
  const { locale } = await params;
  const alternates = getAlternateLanguages(locale, { pathname: "/plans" });
...
export default async function PublicPlansPage(props: { searchParams: SearchParams }) {
  const t = await getTranslations("PublicPlansPage");
  ...
  const plansData = await getPublicPlansWithBookmarkStatus(currentPage, pageSize, searchTerm);
  ...
  return (
    <main className="min-h-100vh-minus-header flex flex-col justify-center items-center gap-8 py-10">
      <div className="text-center space-y-4 max-w-4xl mx-auto text-pretty">
        <ReducableViewTransition name="public-plans-text">
      ...
      <div className="w-between-95-percent-and-1440 mx-auto">
        <AnimatedFormBox name="form-container" className="grid place-items-center w-full">
          <PublicPlansTable
            ...
            isAuthenticated={plansData.isAuthenticated}
          />
        </AnimatedFormBox>
      </div>
    </main>
