...
import type { Plan, Step, StepGroup } from "@/db/schema";
...
interface EditPlanFormProps {
    plan: Plan & { steps: Step[]; stepGroups: StepGroup[] };
}

export default function EditPlanForm({ plan }: EditPlanFormProps) {
    const t = useTranslations("EditPlanPage.form");
    const tSuccess = useTranslations("EditPlanPage.success");
    const [title, setTitle] = useState(plan.title);
    const [isPublic, setIsPublic] = useState(plan.isPublic);
    const [planItems, setPlanItems] = useState<PlanItem[]>([]);
    const [nextOrder, setNextOrder] = useState(0);

    // Initialize form with existing plan data once
    useEffect(() => {
        const items: PlanItem[] = [];
        const groups = new Map(plan.stepGroups.map(g => [g.id, g]));
        const groupedSteps = new Map<string | null, Step[]>();

        // Group steps by stepGroupId
        plan.steps.forEach(step => {
            const groupId = step.stepGroupId;
            if (!groupedSteps.has(groupId)) {
                groupedSteps.set(groupId, []);
            }
            groupedSteps.get(groupId)!.push(step);
        });

        // Sort standalone steps by order
        const standaloneSteps = (groupedSteps.get(null) || []).sort((a, b) => a.order - b.order);
        const sortedGroups = plan.stepGroups.sort((a, b) => a.order - b.order);

        // Create a combined list maintaining order
        const allItems: Array<{ type: 'step' | 'group', order: number, data: any }> = [];
        ...
        allItems.forEach(item => {
            if (item.type === 'step') {
                ...
                const group = item.data as StepGroup;
                const groupSteps = (groupedSteps.get(group.id) || [])
        ...
        setPlanItems(items);
        setNextOrder(Math.max(...allItems.map(i => i.order), 0) + 1);
    }, [plan]);
    ...
    // Show success card if plan was updated
    if (state?.success) {
        return (
...
// Step row component (same as CreatePlanForm)
function StepRow({
    step,
    index,
    totalItems,
            ...
            <div className="flex-1 space-y-2">
                ...
                <div className="flex gap-2">
                    ...
                    <Input
                        ...
                        max="59"
                        value={step.minutes}
                        onChange={(e) => onUpdate(step.id, "minutes", parseInt(e.target.value) || 0)}
                    ...
                    <Input
                        ...
                        max="59"
                        value={step.seconds}
                        onChange={(e) => onUpdate(step.id, "seconds", parseInt(e.target.value) || 0)}
...
// Step group row component (same as CreatePlanForm)
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <div className="flex gap-0.5">
                                        <Input
                                            ...
                                            max="59"
                                            value={step.minutes}
                                            onChange={(e) => onUpdateStep(step.id, "minutes", Math.min(59, Math.max(0, parseInt(e.target.value) || 0)))}
                                            placeholder={t("minutesPlaceholder")}
                                            className="w-20"
                                        />
                                    ...
                                    <div className="flex gap-0.5">
                                        <Input
                                            ...
                                            max="59"
                                            value={step.seconds}
                                            onChange={(e) => onUpdateStep(step.id, "seconds", Math.min(59, Math.max(0, parseInt(e.target.value) || 0)))}
                                            placeholder={t("secondsPlaceholder")}
                                            className="w-20"
                                        />
